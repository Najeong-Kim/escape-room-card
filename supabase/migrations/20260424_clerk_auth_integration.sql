-- Allow user-owned tables to work with Clerk user IDs via Supabase third-party auth.

alter table if exists user_room_logs
  drop constraint if exists user_room_logs_user_id_fkey;

alter table if exists user_card_profiles
  drop constraint if exists user_card_profiles_user_id_fkey;
...
as $$
  ...
  liked as (
    ...
    where profiles.character_id = p_character_id
      and logs.rating >= 4
      and ((auth.jwt()->>'sub') is null or logs.user_id <> (auth.jwt()->>'sub'))
      and not exists (
        select 1
