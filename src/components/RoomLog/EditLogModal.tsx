...
import { updateLog } from '../../lib/roomLog'
...
export function EditLogModal({ log, onClose, onSaved }: Props) {
  const today = new Date().toISOString().slice(0, 10)
  const [playedAt, setPlayedAt] = useState(log.played_at)
  const [cleared, setCleared] = useState(log.cleared)
  const [rating, setRating] = useState<PathRating | null>(log.rating)
  const [memo, setMemo] = useState(log.memo)
  const [metricScores, setMetricScores] = useState<MetricScores>({
    difficulty: log.difficulty_score ?? undefined,
    fear: log.fear_score ?? undefined,
    activity: log.activity_score ?? undefined,
    story: log.story_score ?? undefined,
    interior: log.interior_score ?? undefined,
    aging: log.aging_score ?? undefined,
  })
        ...
        >
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="text-white font-bold text-lg leading-snug">{log.room_name}</h2>
            </div>
            <button
              onClick={onClose}
